...
local функция   must_ignore_window  (  win_id )
  локальный   идентификатор buf_id  = vim.api.nvim_win_get_buf(win_id)
  местный  filetype = vim.api.nvim_buf_get_option(buf_id, "filetype")
  для  _, ft in ipairs(M.config.ignore_filetypes) do
    если   тип файла  == ft then
      возвращаться  true
    end
  end
   возврата  false
...
local функция   find_ignored_window  () 
  для   _win_id , ​  in ipairs(vim.api.nvim_list_wins()) do
    местный  buf_id = vim.api.nvim_win_get_buf(win_id)
    местный  filetype = vim.api.nvim_buf_get_option(buf_id, "filetype")
    слишком  _, ft in ipairs(M.config.ignore_filetypes) do
      если  filetype == ft then
        return win_id
      end
    end
  end
